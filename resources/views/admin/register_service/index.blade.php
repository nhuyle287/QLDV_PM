...
@section('content')
    <div class="content">
        <h1 class="title" id="title">Danh sách dịch vụ/phần mềm đang sử dụng</h1>
        <div class="row">
            @if(session('success'))
        ...
        <div class="panel panel-default">
            <form method="get" class="search_frm" action="{{route('admin.list-services.search')}}">
                @csrf
                        ...
                        <div class="card-header" role="tab" id="heading79">
                            ...
                            <div style="float: left" class="dropdown">
                                ...
                                <div class="dropdown-menu">
                                    <a class="dropdown-item" id="domain" data-id="0"
                                       href="{{route('admin.list-services.domainusing')}}">Tên
                                        miền</a>
                                    <a class="dropdown-item" id="hosting" data-id="1"
                                       href="{{route('admin.list-services.hostingusing')}}">Hosting</a>
                                    <a class="dropdown-item" id="vps" data-id="2"
                                       href="{{route('admin.list-services.vpsusing')}}">VPS</a>
                                    <a class="dropdown-item" id="web" data-id="3"
                                       href="{{route('admin.list-services.websiteusing')}}">Website</a>
                                    <a class="dropdown-item" data-id="4" id="email"
                                       href="{{route('admin.list-services.emailusing')}}">Email</a>
                                    <a class="dropdown-item" id="soft" data-id="5"
                                       href="{{route('admin.list-services.softusing')}}">Phần
                                        mềm</a>
                                </div>
                            </div>
                        </div>
                            ...
                            @if($register_softs!==null)
                                @forelse($register_softs as $register_soft)
                                    <tr>
                                        ...
                                        @endif
                                        <td class="thstyleform">
                                            <a href="{{route('admin.register-softs.show', $register_soft->id)}}"
                                               class="btn btn-xs btn-info">{{ __('general.view') }}</a>
                                            <a href="{{route('admin.register-softs.edit', [$register_soft->id])}}"
                                               class="btn btn-xs btn-success">{{ __('general.edit') }}</a>
                                            <form style="display: inline-block"
                                                  action="{{route('admin.register-softs.destroy', [$register_soft->id])}}"
                                                  method="post"
                                                  onsubmit="return confirm('Bạn có chắc chắn muốn xóa?');">
                                                @csrf
                                                <button type="submit"
